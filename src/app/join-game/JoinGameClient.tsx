...
export default function JoinGameClient() {
  ...
  const initialGameId = useMemo(() => searchParams.get("gameId") ?? "", [searchParams]);
  ...
  // If the URL changes client-side (rare here), keep the input in sync.
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen">
      <div className="max-w-xl space-y-4">
        ...
        <p className="text-sm text-gray-600">Enter the Game ID shared by the host.</p>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {info && (
            <div className="rounded-2xl border p-4 text-sm">
              {info.ok ? (
                <div className="text-green-700">✅ Joinable — {info.current}/{info.max} players</div>
              ) : (
